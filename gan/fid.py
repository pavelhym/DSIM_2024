...
directory_path = "epoch_images"
try:
    # List all entries in the directory
    entries = os.listdir(directory_path)
    
    # Filter out the directories from the entries list
...
for epoch in range(epochs):
    fid_value = calculate_fid_given_paths(
        [f"epoch_images/{epoch}", "real_images"], 60, 'cpu', 2048, num_workers=0)
...
plt.plot(fid_values, linewidth=4) 
#plt.title("FID", style='italic', size=25) 
plt.xlabel("epoch", style='italic', size=18)
plt.ylabel("FID", style='italic', size=18)
plt.yticks(size=16)
plt.xticks(size=16)

plt.grid(True) 
plt.savefig('fid_graph.png', dpi=300, format='png' )
